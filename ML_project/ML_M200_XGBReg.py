...
def XGB_model(df=None, feature_list=None, use_pca=True, use_optimal_features=False,
              pca_threshold=0.90, Ngal_threshold=0, regression_plot=True, importances_plot=True,
              test_size=0.3, n_trees=200, random_state=42, max_depth=10, learning_rate=0.1, 
              subsample=0.8, colsample_bytree=1.0, tree_method='hist',
              threshold_width=0.5, bin_width=0.1, method='fixed', min_bin_count=10):
    '''
    Train and evaluate a XGBoost Regressor model to predict halo mass (log(M200)) for galaxy groups.
    ...
    learning_rate : float, default=0.1
        Scaling factor for each tree's contribution to the entire model.
        Smaller values --> model learns a bit more slowly and may require more trees -- but also more robust!
    subsample : float, default = 0.8
        Fraction of the training rows used to build each tree (row sampling without replacement).
    colsample_bytree : float, default = 1.0
        Fraction of features used to build each tree.
    tree_method : str, default = 'hist'
        The algorithm which XGBoost uses to build the trees. 'hist' is an acceptable default for medium-large datasets.
    ...
    df_out = df.copy()
    if use_pca:
        df_out, feature_list = get_pca_features(df, feature_list, pca_threshold)
    ...
    #objective == the loss function the model uses to optimize itself
        #reg squared error is a direct analogue to random forest regression
    #learning_rate == scaling factor for each tree's contribution to the entire model
        #smaller values -- model learns a bit more slowly and may require more trees -- but also more robust!
    #model = XGBRegressor(objective='reg:squarederror', n_estimators=n_trees, max_depth=max_depth, 
    #                     learning_rate=learning_rate, subsample=subsample, colsample_bytree=colsample_bytree, 
    #                     random_state=random_state, tree_method=tree_method)
...
if __name__ == "__main__":
    ...
    _ = XGB_model(df=df, 
                  ...
                  use_pca=bool(int(param_dict['use_pca'])), 
                  ...
                  colsample_bytree=float(param_dict['colsample_bytree']),
                  ...
                  min_bin_count=float(param_dict['min_bin_count']), 
                  method=str(param_dict['method']),
