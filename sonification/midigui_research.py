...
##########
    ...
    def midi_setup_bar(self):
        ...
        if int(self.var_w1w3.get()) > 0:
            ...
            self.band_alt = 'W3' if ((self.band == 'W1') or ('W1' in self.path_to_im.get())) else 'W1'

            alt_path = alt_path = str(self.path_to_im.get()).replace(self.band,self.band_alt)
            self.dat_alt = fits.getdata(alt_path)

            self.dat_alt *= self.mask_bool
    ...
    def create_midi_animation(self):
        ...
        fps = len(self.xvals_anim) / self.length_of_file
        print("Frames:", len(self.xvals_anim))
        print("FPS:", fps)
